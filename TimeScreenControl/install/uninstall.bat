...
if exist "%STARTMENU_DIR%" (
    ...
    echo ?? Ярлыки пользователя не найдены
)
...
if exist "%INSTALL_DIR%" (
    ...
    echo ?? Каталог программы не найден
)
echo.
...
if /i "!preserve!"=="Y" (
    ...
    if exist "%CONFIG_DIR%" (
        ...
        if errorlevel 1 (
            echo ?? Не удалось удалить конфигурацию (файл может быть открыт)
